...
Session topology can be introspected via `deviceCount`, lexicographically sorted `registeredDeviceIds`, snapshot helpers (`deviceSnapshot`, `allDeviceSnapshots`), and strict lookup (`tryDeviceSnapshot`) for stable live routing diagnostics.
