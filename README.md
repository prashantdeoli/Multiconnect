...
Session topology can be introspected via `deviceCount`, lexicographically sorted `registeredDeviceIds`, snapshot helpers (`deviceSnapshot`, `allDeviceSnapshots`), and a one-shot `sessionSnapshot` aggregate for stable live routing diagnostics.
