...
- `scripts/doctor_local_setup.py` — preflight checker for local prerequisites (`python`/`python3`, `cmake`, `ctest`, repo paths).
- `scripts/close_day1_blockers.py` — one-command helper to persist Day-1 acceptance evidence and regenerate blocker/status outputs.
...
Before first run on a new machine, verify prerequisites with:

```bash
python3 scripts/doctor_local_setup.py
```
...
On macOS where only `python3` is installed, the script auto-detects it (or set `PYTHON_BIN=python3`).
...
Fastest way to close both remaining Day-1 blockers after manual validation:

```bash
python3 scripts/close_day1_blockers.py \
  --audio-status Pass \
  --audio-evidence "No crackle in 3/3 30-min physical runs (MC-001..MC-003)." \
  --audio-next-step "Track weekly regressions in hardware matrix notes." \
  --ui-status Pass \
...
Use `python3 scripts/close_day1_blockers.py --refresh-only` to regenerate blockers/status reports from existing config without running full native build/test.
