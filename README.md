...
Session topology can be introspected via `deviceCount`, lexicographically sorted `registeredDeviceIds`, and snapshot helpers (`deviceSnapshot`, `allDeviceSnapshots`) for stable live routing diagnostics; snapshots include a `registered` flag so callers can distinguish unknown-device lookups from valid zero-valued runtime state.
