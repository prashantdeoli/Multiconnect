...
DeviceSnapshot SyncEngine::deviceSnapshot(const std::string& deviceId) const {
    ...
    (void)tryDeviceSnapshot(deviceId, &snapshot);
    return snapshot;
}

bool SyncEngine::tryDeviceSnapshot(const std::string& deviceId, DeviceSnapshot* outSnapshot) const {
    ...
    outSnapshot->state = stateIt->second;

    const auto metricsIt = metrics_.find(deviceId);
    if (metricsIt != metrics_.end()) {
        outSnapshot->metrics = metricsIt->second;
    } else {
        outSnapshot->metrics = DeviceStreamMetrics{};
    }
