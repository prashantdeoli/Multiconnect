...
int main() {
    ...
    multiconnect::DeviceSnapshot sonySnapshotTry;
    assert(engine.tryDeviceSnapshot("sony", &sonySnapshotTry));
    assert(sonySnapshotTry.deviceId == "sony");
    assert(sonySnapshotTry.state.offsetSamples == 0);

    multiconnect::DeviceSnapshot missingSnapshotTry;
    assert(!engine.tryDeviceSnapshot("missing", &missingSnapshotTry));
    assert(!engine.tryDeviceSnapshot("sony", nullptr));
