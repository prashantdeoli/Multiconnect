...
int main() {
    ...
    const auto sessionSnapshot = engine.sessionSnapshot();
    assert(sessionSnapshot.bufferedSamples == engine.bufferedSamples());
    assert(sessionSnapshot.devices.size() == 2);
    assert(sessionSnapshot.devices[0].deviceId == "sony");
    assert(sessionSnapshot.devices[1].deviceId == "tribit");
    ...
    const auto postUnregisterSession = engine.sessionSnapshot();
    assert(postUnregisterSession.devices.size() == 1);
    assert(postUnregisterSession.devices[0].deviceId == "tribit");
